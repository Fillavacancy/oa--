...
public class SelUserAllServlet extends HttpServlet
{
    private static final long serialVersionUID = 174772398969904592L;
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        else
        {
            ...
            try
            {
                List list = userMail.infoUserAll();
                if (list.size() > 0)
                {
                    // 排除自己的信息
                    ...
                    // 排除已经是自己的好友信息
                    List<Personalbook> Personalbooklist = userMail.infoPersonalbooks(userId);
                    if (Personalbooklist.size() > 0)
                    {
                        for (Personalbook personalbook : Personalbooklist)
                        {
                            for (User user : list2)
                            {
                                if (user.getUserId().equals(personalbook.getFriendId()))
                                {
                                    list2.remove(user);
                                    break;
                                }
